...
<?php
	...
	function select_data($user="") {
		...
		if ($user != "") $sql = "SELECT * FROM mahasiswa WHERE NIM = :user";
		else $sql = "SELECT * FROM mahasiswa";
	...
	function insert_data($data) {
		...
		if ($data != null) {
			try {
				$sql = "INSERT INTO mahasiswa VALUES (:nim, :nama, :ipk, :asal)";
				$stmt = $con->prepare($sql);
				...
				$sql = "INSERT INTO mahasiswa VALUES (:nim, :pass)";
				$stmt = $con->prepare($sql);
				$stmt->bindValue(':nim', $data['nim'], PDO::PARAM_STR);
	...
	function update_data($nim="",$data) {
		...
		if ($data != null) {
			try {
				$sql = "UPDATE mahasiswa SET Nama = :nama, IPK = :ipk, Asal = :asal WHERE NIM = :nim";
				$stmt = $con->prepare($sql);
	...
	function delete_data($nim="") {
		...
		if ($nim != "") {
			try {
				...
				$sql = "DELETE FROM login_mhs WHERE NIM = :nim";
				$stmt = $con->prepare($sql);
				$stmt->bindValue(':nim', $nim, PDO::PARAM_STR);
				if ($stmt->execute()) return true;
				else return false;
